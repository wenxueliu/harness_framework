...
| `--acp-agent` | 可选 ACP Agent 覆盖 | `claude` 或 `codex` |
...
未提供 `--acp-agent` 时，框架按任务类型自动选择 Agent。

> **禁止行为**：不得自动生成 `req_id`、`task_name`。
...
| `--acp-agent` | 否 | 覆盖默认 ACP Agent：`claude` 或 `codex` |
| `--agent-name` | 否 | 旧注册/抢占 Worker 的兼容字段 |
...
python scripts/add_task.py req-001 deploy-canary \
  ...
  --type deploy \
  --depends-on build --acp-agent codex --service-name platform
```
...
4. 如果初始状态是 PENDING，Aggregator 会看到它并等待依赖满足
   如果初始状态是 BLOCKED，上下游任务完成后自动激活
       │
       ▼
5. ACPDispatcher 创建对应类型的 Claude/Codex Agent 执行新任务
```
