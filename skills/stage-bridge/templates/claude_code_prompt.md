...
# 2. 启动执行 Session（记录会话生命周期）
SESSION_ID="${AGENT_ID}-$(date +%s)"
python $STAGE_BRIDGE_DIR/scripts/record_session_start.py "$REQ_ID" "$TASK_NAME" "$SESSION_ID"
export SESSION_ID
...
4. **真正的编码工作**：使用 Read / Edit / Write / Bash 工具在 `$REPO_PATH` 下完成需求实现。每完成一个用户可感知的步骤（创建分支、实现一个接口、通过单元测试），调用一次 `log_step.py`。
...
```bash
# 先关闭 Session
python $STAGE_BRIDGE_DIR/scripts/record_session_end.py "$REQ_ID" "$TASK_NAME" "$SESSION_ID" \
  --status completed
